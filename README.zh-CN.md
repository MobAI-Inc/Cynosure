...
Cynosure 的候选模型池可通过 `routes` 配置，Jev 根据任务与执行反馈选择模型，可以先尝试一个候选，再追加比较或交付结果。

本次[评测配置](eval/runtime.json)使用 **Grok 4.6、DeepSeek V4.1 Flash、GLM 5.3、GLM 5.3 Flash** 作为生成候选，并分别以这四个模型作为单模型对照。`typesafe/jev-1.13` 负责路由决策，`jina-embeddings-v5-text-small` 与 SQLite 全文检索提供经验检索。
...
| 单独使用 DeepSeek V4.1 Flash | 31/48 | 64.6% | 71 |
| 单独使用 Grok 4.6 | 22/48 | 45.8% | 53 |

### 效果、成本与速度

![Cynosure 效果、预计费用与速度](docs/assets/benchmark-tradeoffs.png)

**Cynosure 在本次对照中通过率为 85.4%，每次成功任务预计费用为 $0.0368，成功任务中位耗时为 77.7 秒。**

![效果与费用的观测帕累托前沿，星标为 Cynosure](docs/assets/benchmark-frontier.png)

纵轴以本次任务通过率衡量效果，左上方向代表更高通过率与更低预计费用。
