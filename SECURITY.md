...
Experience contains actual prompts and outputs. Protect the local data directory and bind each trusted caller to its own scope. Scope filtering is not authentication. The CLI has no public network listener; any future HTTP host must enforce identity and permissions before calling it.
